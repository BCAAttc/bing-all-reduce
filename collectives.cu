...
void InitCollectives(int device) {
    if(device < 0) {
		...
		std::cerr << "cudaError_t " << error << std::endl;
...
void RingAllreduce(float* data, size_t length, float** output_ptr) {
    ...
    for(size_t other_length : lengths) {
		...
		//std::cerr << "other_length " << other_length << std::endl;
		//std::cerr << "lengths " << length << std::endl;
    ...
    // chunks, where N is the MPI size. 矩阵切分成N份， N为节点数量
    ...
    *output_ptr =  output;
    ...
    for (int i = 0; i < size - 1; i++) {
		...
		/*
		if (rank==1){
			std::cerr << "output " << segment_ends[send_chunk] -segment_sizes[send_chunk] << std::endl;
			std::cerr << "update " << segment_ends[recv_chunk] -segment_sizes[recv_chunk] << std::endl;
...
void RingAllreduce_1(float* data, size_t length, float** output_ptr) {
    // Get MPI size and rank.  数组 数组大小 输出的东西
    int rank; // 编号
    int mpi_error = MPI_Comm_rank(MPI_COMM_WORLD, &rank); //获取自己的 rank
    if(mpi_error != MPI_SUCCESS)
        throw std::runtime_error("MPI_Comm_rank failed with an error");

    int size; //通信数据大小
    mpi_error = MPI_Comm_size(MPI_COMM_WORLD, &size);  //获取通信的数据大小 size
    if(mpi_error != MPI_SUCCESS)
        throw std::runtime_error("MPI_Comm_size failed with an error");

    // Check that the lengths given to every process are the same.//确定节点数和实际可用的节点一致
    std::vector<size_t> lengths = AllgatherInputLengths(size, length);
    for(size_t other_length : lengths) {
        if(length != other_length) {
            throw std::runtime_error("RingAllreduce received different lengths");
    ...
    // Partition the elements of the array into N approximately equal-sized
    // chunks, where N is the MPI size. 矩阵切分成N份， N为节点数量 segment_size为每一份的大小
    const size_t segment_size = length / size;
    std::vector<size_t> segment_sizes(size, segment_size); // 矩阵（size*segment_size）（5*5111406）
	//std::cerr << "segment_size： " << segment_size << std::endl;
    const size_t residual = length % size;
    ...
    *output_ptr =  output;
    ...
    // Copy your data to the output buffer to avoid modifying the input buffer. 创建输出矩阵
    copy(output, data, length);

    // Allocate a temporary buffer to store incoming data. 创建一个缓冲矩阵buffer，用来接收数据
    // We know that segment_sizes[0] is going to be the largest buffer size,
    // because if there are any overflow elements at least one will be added to
    // the first segment.
    float* buffer_1 = alloc(segment_sizes[0]);
	float* buffer_2 = alloc(segment_sizes[0]);
    // Receive from your left neighbor with wrap-around. 左侧节点 recv_from
    const size_t recv_from = (rank - 1 + size) % size;

    // Send to your right neighbor with wrap-around. 右侧节点 send_to
    const size_t send_to = (rank + 1) % size;
	/*
	if (rank==0){
		std::cerr << "recv_from： " << recv_from << std::endl;
		std::cerr << "send_to： " << send_to << std::endl;
	}*/
    MPI_Status recv_status_1;
	MPI_Status recv_status_2;
	MPI_Status send_status_1;
	MPI_Status send_status_2;
	
    MPI_Request recv_req_1,send_req_1;
	MPI_Request recv_req_2,send_req_2;
    MPI_Datatype datatype = MPI_FLOAT;
    ...
    // segment (rank - i - 1).
    for (int i = 0; i < size - 1; i++) {
        int recv_chunk_1 = (rank - i - 1 + size) % size; //1
        int send_chunk_1 = (rank - i + size) % size;     //1
		    int recv_chunk_2 = (rank + i + size + 2) % size;
        int send_chunk_2 = (rank + i + size + 1) % size;
		/*
		if (rank==1){
			std::cerr << "send_chunk_1 " << send_chunk_1 << std::endl;
			std::cerr << "send_chunk_2 " << send_chunk_2 << std::endl;
			std::cerr << "recv_chunk_1 " << recv_chunk_1 << std::endl;
			
			std::cerr << "recv_chunk_2 " << recv_chunk_2 << std::endl;
			
			}
		*/
        float* segment_send_1 = &(output[segment_ends[send_chunk_1] -
                                   segment_sizes[send_chunk_1]]);
								   
		// 第一次通信 异步发送
    MPI_Irecv(buffer_1, segment_sizes[recv_chunk_1],datatype, recv_from, 0, MPI_COMM_WORLD, &recv_req_1);

        //MPI_Send(segment_send_1, segment_sizes[send_chunk_1],MPI_FLOAT, send_to, 0, MPI_COMM_WORLD);
		MPI_Isend(segment_send_1, segment_sizes[send_chunk_1],MPI_FLOAT, send_to, 0, MPI_COMM_WORLD, &send_req_1);		
	
		float* segment_send_2 = &(output[segment_ends[send_chunk_2] -
                                   segment_sizes[send_chunk_2]]);
		
		// 第二次通信 异步发送
		MPI_Irecv(buffer_2, segment_sizes[recv_chunk_2],
                datatype, send_to, 0, MPI_COMM_WORLD, &recv_req_2);

        //MPI_Send(segment_send_2, segment_sizes[send_chunk_2],MPI_FLOAT, recv_from, 0, MPI_COMM_WORLD);
		MPI_Isend(segment_send_2, segment_sizes[send_chunk_2],MPI_FLOAT, recv_from, 0, MPI_COMM_WORLD,&send_req_2);		
				
        
    float *segment_update_1 = &(output[segment_ends[recv_chunk_1] -
                                         segment_sizes[recv_chunk_1]]);
		float *segment_update_2 = &(output[segment_ends[recv_chunk_2] - segment_sizes[recv_chunk_2]]);
		
		// 第二个计算
		/*
		if (i>1 && i < (size-1)/2+1){
			reduce(segment_update_2, buffer_2, segment_sizes[recv_chunk_2]);
			}
		*/
		
        // Wait for recv to complete before reduction 第一个等待		
        MPI_Wait(&recv_req_1, &recv_status_1);
		//MPI_Wait(&send_req_1, &send_status_1);
		/*
		if (i < (size-1)/2){
			reduce(segment_update_1, buffer_1, segment_sizes[recv_chunk_1]);
			}
		*/
		...
		*/
		...
		// 第二个等待
		MPI_Wait(&recv_req_2, &recv_status_2);
		//MPI_Wait(&send_req_2, &send_status_2);
		/*
		if (i < (size-1)/2){
			reduce(segment_update_2, buffer_2, segment_sizes[recv_chunk_2]);
			}
		*/
		...
		/*
		if (rank==0){
			std::cerr << "update： " << segment_ends[recv_chunk_1] -  segment_sizes[recv_chunk_1] << std::endl;

			std::cerr << "update： " << segment_ends[recv_chunk_2] -  segment_sizes[recv_chunk_2] << std::endl;
			}	
		*/
    }
	MPI_Wait(&send_req_2, &send_status_2);
...
void RingAllreduce_nn(float* data, size_t length, float** output_ptr) {
    // Get MPI size and rank.  数组 数组大小 输出的东西
    int rank; // 编号
    int mpi_error = MPI_Comm_rank(MPI_COMM_WORLD, &rank); //获取自己的 rank
    if(mpi_error != MPI_SUCCESS)
    ...
    *output_ptr =  output;
    ...
    // because if there are any overflow elements at least one will be added to
    // the first segment.
    float* buffer_1 = alloc(segment_sizes[0]*2);
	float* buffer_2 = alloc(segment_sizes[0]*2);
    // Receive from your left neighbor with wrap-around. 左侧节点 recv_from
    const size_t recv_from = (rank - 1 + size) % size;
    ...
    for (int i = 0; i < 2*(size - 1); i++) {
        int recv_chunk_1 = (rank - i - 1 + size*2) % size; //1
        int send_chunk_1 = (rank - i + size*2) % size;     //1
		int recv_chunk_2 = (rank + i + size*2 + 2) % size;
        int send_chunk_2 = (rank + i + size*2 + 1) % size;
		/*
		if (rank==1){
			std::cerr << "send_chunk_1 " << send_chunk_1 << std::endl;
			std::cerr << "send_chunk_2 " << send_chunk_2 << std::endl;
		...
		*/
		...
		/*
		if (rank==0){
			std::cerr << "output： " << segment_ends[send_chunk_1] -  segment_sizes[send_chunk_1] << std::endl;

			std::cerr << "output2： " << segment_ends[send_chunk_2] -  segment_sizes[send_chunk_2] << std::endl;
			}			   
		*/
...
void RingAllreduce_n(float* data, size_t length, float** output_ptr) {
    // Get MPI size and rank.  数组 数组大小 输出的东西
    int rank; // 编号
    int mpi_error = MPI_Comm_rank(MPI_COMM_WORLD, &rank); //获取自己的 rank
    if(mpi_error != MPI_SUCCESS)
    ...
    // Partition the elements of the array into N approximately equal-sized
    // chunks, where N is the MPI size. 矩阵切分成N份， N为节点数量 segment_size为每一份的大小
    const size_t segment_size = length / (2*size);
    std::vector<size_t> segment_sizes(size, segment_size); // 矩阵（size*segment_size）（5*5111406）
	//std::cerr << "segment_size： " << segment_size << std::endl;
    const size_t residual = length % size;
	//std::cerr << "residual： " << residual << std::endl;
