...
void TestCollectivesCPU(std::vector<size_t>& sizes, std::vector<size_t>& iterations) {
    ...
    for(size_t i = 0; i < sizes.size(); i++) {
        ...
        for(size_t iter = 0; iter < iters; iter++) {
            ...
            float* output;
            timer.start();
            //std::cerr << "ck1 " << size << std::endl;
            //std::cerr << "ck2 " << data << std::endl;
...
void TestCollectivesCPU_1(std::vector<size_t>& sizes, std::vector<size_t>& iterations) {
    // Initialize on CPU (no GPU device ID).
    InitCollectives(NO_DEVICE);
    ...
    for(size_t i = 0; i < sizes.size(); i++) {
        ...
        for(size_t iter = 0; iter < iters; iter++) {
            ...
            //std::cerr << "ck1 " << size << std::endl;
            //std::cerr << "ck2 " << data << std::endl;
            RingAllreduce_1(data, size, &output);
            seconds += timer.seconds();
...
void TestCollectivesCPU_n(std::vector<size_t>& sizes, std::vector<size_t>& iterations) {
    // Initialize on CPU (no GPU device ID).
    InitCollectives(NO_DEVICE);
    ...
    for(size_t i = 0; i < sizes.size(); i++) {
        ...
        for(size_t iter = 0; iter < iters; iter++) {
            ...
            //std::cerr << "ck1 " << size << std::endl;
            //std::cerr << "ck2 " << data << std::endl;
            RingAllreduce_n(data, size, &output);
            seconds += timer.seconds();
...
int main(int argc, char** argv) {
    ...
    if(input == "cpu_1") {
        ...
        TestCollectivesCPU_1(buffer_sizes, iterations);
    }else if (input == "cpu_n"){
        TestCollectivesCPU_n(buffer_sizes, iterations);
    }
    else if(input == "gpu") {
